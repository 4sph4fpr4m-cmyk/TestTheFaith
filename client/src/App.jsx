...
const API_BASE = "https://testthefaith.com";
...
export default function App() {
  ...
  async function handleAsk(q) {
    ...
    try {
      ...
      setCards((prev) => [{ q: query, data }, ...prev]);
    } catch (e) {
  ...
  async function handleDebate(q) {
    const query = q || question;
    if (!query.trim() || loading) return;
    setLoading(true);
    setQuestion("");
    const newHistory = [...debateHistory, { role: "user", content: query }];
    setDebateMessages((prev) => [...prev, { role: "user", text: query }]);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0f1724", color: "#e8dfc0", fontFamily: "Georgia, serif" }}>
      ...
      {/* Header */}
      <div style={{ background: "#1a2744", padding: "16px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: "0.5px solid #ffffff18", position: "sticky", top: 0, zIndex: 10 }}>
      ...
      <div style={{ maxWidth: 760, margin: "0 auto", padding: "0 16px 40px" }}>
        ...
        {mode === "ask" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
            {cards.length === 0 && !loading && (
              <div style={{ textAlign: "center", color: "#7a8fb0", padding: "60px 20px", lineHeight: 1.8 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8, maxWidth: 400, margin: "24px auto 0" }}>
                  {[
                    "Dawkins says evolution makes God unnecessary",
                    "Ehrman says the Gospels contradict each other",
                    "The problem of evil makes God impossible",
                    "We don't need God to have objective morality",
                    "Hawking says physics shows the universe created itself",
                  ].map((s, i) => (
                    <button key={i} onClick={() => handleAsk(s)} style={{ textAlign: "left", padding: "10px 14px", fontSize: 13, borderRadius: 8, border: "0.5px solid #ffffff18", background: "#1a2744", color: "#a0b0cc", cursor: "pointer", fontFamily: "Georgia, serif" }}>
            ...
            {loading && (
              <div style={{ textAlign: "center", color: "#7a8fb0", padding: 40, fontSize: 14, fontStyle: "italic" }}>Consulting the tradition…</div>
            )}
            ...
            {cards.map((card, i) => (
              <div key={i} style={{ border: "0.5px solid #ffffff18", borderRadius: 12, overflow: "hidden" }}>
                  ...
                  {(card.data.responses || []).map((r, j) => (
                    <div key={j} style={{ background: "#1a2744", border: "0.5px solid #ffffff12", borderRadius: 10, padding: "12px 14px" }}>
                      ...
                      {r.source_url ? (
                        ...
                        <span style={{ fontSize: 11, color: "#7a8fb0" }}>{r.source_title}</span>
                      ) : null}
                    </div>
                  ))}
                </div>
                ...
                <div style={{ padding: "14px 18px" }}>
                    ...
                    <>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 16 }}>
                        {card.data.follow_ups.map((f, j) => (
                          <button key={j} onClick={() => handleAsk(f)} style={{ textAlign: "left", padding: "8px 12px", fontSize: 13, borderRadius: 8, border: "0.5px solid #ffffff18", background: "none", color: "#a0b0cc", cursor: "pointer", lineHeight: 1.5, fontFamily: "Georgia, serif" }}>
                  ...
                  {/* Inline push-back input */}
                  <div style={{ borderTop: "0.5px solid #ffffff10", paddingTop: 14 }}>
                    <div style={{ fontSize: 10, fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase", color: "#7a8fb0", marginBottom: 8 }}>Push Back or Go Deeper</div>
                    <InlineInput onSubmit={(val) => handleAsk(val)} />
                  </div>
                </div>
        ...
        {mode === "debate" && (
          <div>
              ...
              <div style={{ textAlign: "center", color: "#7a8fb0", padding: "60px 20px", lineHeight: 1.8 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8, maxWidth: 400, margin: "0 auto" }}>
                  {[
                    ...
                    <button key={i} onClick={() => { setQuestion(s); handleDebate(s); }} style={{ textAlign: "left", padding: "10px 14px", fontSize: 13, borderRadius: 8, border: "0.5px solid #ffffff18", background: "#1a2744", color: "#a0b0cc", cursor: "pointer", fontFamily: "Georgia, serif" }}>
                      <span style={{ color: "#c9a84c", marginRight: 6 }}>→</span>{s}
                    </button>
                  ))}
                </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              {loading && (
                <div style={{ color: "#7a8fb0", fontSize: 13, padding: "8px 4px", fontStyle: "italic" }}>The Faith is considering…</div>
              )}
  ...
  return (
    <div style={{ display: "flex", gap: 8 }}>
      <textarea
        ...
        placeholder="Challenge a response, push back, or ask a follow-up…"
        rows={2}
        style={{ flex: 1, fontSize: 13, padding: "8px 11px", borderRadius: 8, border: "0.5px solid #ffffff22", background: "#0f1724", color: "#f0e6c8", outline: "none", fontFamily: "Georgia, serif", resize: "none", lineHeight: 1.5 }}
      />
      <button
        onClick={submit}
        style={{ padding: "8px 14px", background: "#c9a84c", color: "#1a2744", border: "none", borderRadius: 8, cursor: "pointer", fontSize: 14, fontWeight: 700, fontFamily: "Georgia, serif", alignSelf: "flex-end" }}
      >↗</button>
    </div>
  );
}
